...
export default function AddNoteScreen({ navigation }) {
  ...
  const accessOptions = [
    ...
    { value: 'class', label: 'Sinif', desc: userClassCode || 'Sinif yok', color: '#f59e0b' },
  ...
  const addFolder = () => {
    ...
    const newFolder = { name, icon: 'folder-outline' };
  ...
  const removeCustomFolder = (name) => {
    const defaults = ['Genel', 'Matematik', 'Fizik', 'Kimya', 'Tarih'];
  ...
  const handleSave = async () => {
    ...
    if (access === 'class' && !userClassCode) {
      Alert.alert('Hata', 'Sinif notu eklemek icin sinif kodunuz olmali!');
      return;
    }
    setLoading(true);
    try {
      await addDoc(collection(db, 'notes'), {
        ...
        classCode: access === 'class' ? userClassCode : '',
        version: 1,
        favorite: false,
  ...
  return (
    <ScrollView style={s.container} showsVerticalScrollIndicator={false}>
      <View style={s.header}>
        ...
        <Text style={s.headerTitle}>Yeni Not</Text>
        <TouchableOpacity onPress={handleSave} style={s.saveBtn} disabled={loading}>
          <Text style={s.saveBtnText}>{loading ? '...' : 'Kaydet'}</Text>
        </TouchableOpacity>
      </View>

      <View style={s.card}>
        <Text style={s.label}>Baslik</Text>
        <TextInput style={s.input} placeholder="Not basligi..." placeholderTextColor={theme.text2} value={title} onChangeText={setTitle} />

        <Text style={s.label}>Icerik</Text>
        <TextInput style={[s.input, s.textarea]} placeholder="Not icerigi..." placeholderTextColor={theme.text2} value={content} onChangeText={setContent} multiline />
        ...
        <View style={s.chipRow}>
          {folders.map(f => (
            <TouchableOpacity
              ...
              onLongPress={() => !defaultFolders.includes(f.name) && removeCustomFolder(f.name)}
            >
        ...
        {userClassCode ? (
          ...
          <View style={s.classInfoRow}>
            ...
            <Text style={[s.classInfoText, { color: '#ef4444' }]}>Sinif kodun yok, sinif notu ekleyemezsin</Text>
          </View>
        )}
          ...
          <View style={s.tagRow}>
            {tags.map(tag => (
              <TouchableOpacity key={tag} style={s.tag} onPress={() => removeTag(tag)}>
                ...
                <Ionicons name="close" size={12} color={theme.blue} />
              </TouchableOpacity>
            ))}
          </View>
        )}
...
const makeStyles = (theme) => StyleSheet.create({
  ...
  tagAddBtn: { backgroundColor: theme.blue, borderRadius: 12, width: 48, height: 48, alignItems: 'center', justifyContent: 'center' },
  tagRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 10 },
  tag: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: theme.blue + '10', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 8 },
  tagText: { fontSize: 12, color: theme.blue, fontWeight: '600' },
});